  ...
  <v-container fluid>
    ...
    <v-card class="mb-4">
      <v-card-text>
        <v-row>
          <v-col cols="12" md="4">
            <v-select v-model="statusFilter" :label="$t('common.status')" :items="statusOptions" clearable hide-details />
          </v-col>
          <v-col cols="12" md="4">
            <v-select v-model="typeFilter" :label="$t('common.type')" :items="typeOptions" clearable hide-details />
          </v-col>
        </v-row>
      </v-card-text>
    </v-card>
      ...
      <v-card-text>
        ...
        >
          ...
          <template #item.actions="{ item }">
            ...
            <v-btn v-if="item.status === 'draft'" icon="mdi-send" size="small" variant="text" color="primary" :title="$t('common.submit')" @click.stop="submitReturn(item)" />
          </template>
        </v-data-table-server>
      </v-card-text>
    </v-card>
      ...
      <v-card>
        ...
        <v-card-text>
          <v-form ref="formRef">
            ...
            <v-row>
              <v-col cols="12" md="6">
                <v-text-field v-model="form.periodStart" :label="$t('common.dateFrom')" type="date" :rules="[rules.required]" />
              </v-col>
              <v-col cols="12" md="6">
                <v-text-field v-model="form.periodEnd" :label="$t('common.dateTo')" type="date" :rules="[rules.required]" />
              </v-col>
            </v-row>
            <div class="d-flex align-center mt-4 mb-2">
              <span class="text-subtitle-1 font-weight-medium">{{ $t('accounting.lineItems') }}</span>
              <v-spacer />
              <v-btn color="primary" variant="outlined" size="small" prepend-icon="mdi-plus" @click="addLineItem">{{ $t('accounting.addLine') }}</v-btn>
            </div>
            <v-table density="compact">
              ...
              <tbody>
                <tr v-for="(line, idx) in form.lines" :key="idx">
                  ...
                  <td style="width: 150px"><v-text-field v-model.number="line.taxCollected" type="number" density="compact" hide-details variant="underlined" class="text-end" /></td>
                  <td style="width: 150px"><v-text-field v-model.number="line.taxPaid" type="number" density="compact" hide-details variant="underlined" class="text-end" /></td>
                  <td><v-btn icon="mdi-close" size="x-small" variant="text" @click="removeLineItem(idx)" /></td>
                </tr>
              </tbody>
              <tfoot>
            ...
            <v-textarea v-model="form.notes" :label="$t('common.notes')" rows="2" class="mt-4" />
          </v-form>
        </v-card-text>
        <v-card-actions>
          <v-spacer />
...
interface TaxReturnLine { description: string; taxCollected: number; taxPaid: number }
interface TaxReturn { _id: string; type: string; periodStart: string; periodEnd: string; status: string; taxCollected: number; taxPaid: number; netPayable: number; notes?: string; lines?: TaxReturnLine[] }
...
const emptyForm = () => ({
  type: '', periodStart: '', periodEnd: '', notes: '',
...
function statusColor(s: string) {
  const colors: Record<string, string> = { draft: 'grey', submitted: 'info', accepted: 'success', rejected: 'error' }
...
const { items, loading, pagination, fetchItems, onUpdateOptions } = usePaginatedTable({
  url: computed(() => `${orgUrl()}/accounting/tax-return`),
  entityKey: 'taxReturns',
  filters,
})

function addLineItem() { form.value.lines.push({ description: '', taxCollected: 0, taxPaid: 0 }) }
function removeLineItem(idx: number) { form.value.lines.splice(idx, 1) }
...
function openDialog(item?: TaxReturn | Record<string, unknown>) {
  if (item && '_id' in item && item._id) {
    ...
    form.value = {
      type: tr.type, periodStart: tr.periodStart?.split('T')[0] || '', periodEnd: tr.periodEnd?.split('T')[0] || '',
      notes: tr.notes || '', lines: tr.lines || [{ description: '', taxCollected: tr.taxCollected, taxPaid: tr.taxPaid }],
    }
  } else {
    editing.value = false
...
async function save() {
  ...
  saving.value = true
  try {
    const payload = { ...form.value, taxCollected: totalCollected.value, taxPaid: totalPaid.value }
    if (editing.value) await httpClient.put(`${orgUrl()}/accounting/tax-return/${selectedId.value}`, payload)
    else await httpClient.post(`${orgUrl()}/accounting/tax-return`, payload)
    await fetchItems()
